...
try {
  ...
  var get_users_args = ["--insecure", "-sS", docker_admin_url + '/_users/_all_docs', '-H', 'Accept: application/json, text/javascript; q=0.01', '-H', 'Accept-Encoding: identity'];
  var get_admins_args = ["--insecure", "-sS", docker_admin_url + '/_config/admins', '-H', 'Accept: application/json, text/javascript; q=0.01', '-H', 'Accept-Encoding: identity'];
  var get_dbs_args = ["--insecure", "-sS", docker_admin_url + '/_all_dbs', '-H', 'Accept: application/json, text/javascript; q=0.01', '-H', 'Accept-Encoding: identity'];
  ...
  __.each(couchdbInfo, function(info, course) {
    if (info.hasOwnProperty('db') && course !== '_admin') {
      ...
      var args = ["--insecure", "-sS", docker_url + '/_design/by_update/_view/default?descending=true&include_docs=true&limit=25&reduce=false', '-H', 'Accept: application/json, text/javascript; q=0.01', '-H', 'Accept-Encoding: identity',
      ...
      if (result !== undefined && !result.hasOwnProperty('error')) {
        ...
        args = ["--insecure", "-sS", "-X", "PUT", docker_admin_url + "/_config/admins/" + username, "-d", '"' + password + '"'];
        run_cmd("curl", args);

        console.log("creating the db...");
        args = ["--insecure", "-sS", "-X", "PUT", docker_url];
        run_cmd("curl", args);
